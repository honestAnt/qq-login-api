...
import com.qq.connect.javabeans.weibo.Education;
import com.qq.connect.javabeans.weibo.Music;
import com.qq.connect.javabeans.weibo.PrivateFlag;
                ...
                try {
                    ...
                    try {
                        ...
                        block40 : {
                            ...
                            this.birthday = new Birthday(data.getInt("birth_year"), data.getInt("birth_month"), data.getInt("birth_day"));
                            ...
                            this.homeCountryCode = data.getString("homecountry_code");
                            this.homePage = data.getString("homepage");
                            this.homeProvinceCode = data.getString("homeprovince_code");
                            ...
                            this.privateFlag = new PrivateFlag(data.getInt("send_private_flag"));
                            ...
                            try {
                                ...
                                this.tweetInfo = new TweetInfo(tjo.getString("city_code"), tjo.getString("country_code"), tjo.getString("emotiontype"), tjo.getString("emotionurl"), tjo.getString("from"), tjo.getString("fromurl"), tjo.getString("geo"), tjo.getString("id"), imageURL, (float)tjo.getDouble("latitude"), tjo.getString("location"), (float)tjo.getDouble("longitude"), new Music(musicAuthorString, musicUrlString, musicTitleString), tjo.getString("origtext"), tjo.getString("province_code"), tjo.getInt("self") == 1, tjo.getInt("status"), tjo.getString("text"), tjo.getLong("timestamp"), tjo.getInt("type"), new Video(picurl, player, realurl, shorturl, title));
                                vjo = null;
                            }
                        }
                        tjo = null;
                        ...
                        try {
                            ...
                            if (tagString != null && !tagString.equals("null")) {
                                throw new QQConnectException("has tag information");
                            }
                        }
                        catch (Exception e) {
                            tagsJSONArray = data.getJSONArray("tag");
                            int tagsNum = tagsJSONArray.length();
                            for (int i = 0; i < tagsNum; ++i) {
                                tag = tagsJSONArray.getJSONObject(i);
                                this.tags.add(new Tag(tag.getString("id"), tag.getString("name")));
                            }
                        }
                        ...
                        try {
                            ...
                            if (companyString != null && !companyString.equals("null")) {
                                throw new QQConnectException("has comp information");
                            }
                        }
                        catch (Exception e) {
                            companysJSONArray = data.getJSONArray("comp");
                            int companyNum = companysJSONArray.length();
                            for (int i = 0; i < companyNum; ++i) {
                                companyJO = companysJSONArray.getJSONObject(i);
                                this.companys.add(new Company(companyJO.getString("begin_year"), companyJO.getString("company_name"), companyJO.getString("department_name"), companyJO.getString("end_year"), companyJO.getString("id")));
                            }
                        }
                        ...
                        try {
                            ...
                            if (eduString != null && !eduString.equals("null")) {
                                throw new QQConnectException("has edu information");
                            }
                        }
                        catch (Exception e) {
                            eduJSONArray = data.getJSONArray("edu");
                            int eduNum = eduJSONArray.length();
                            for (int i = 0; i < eduNum; ++i) {
                                eduJO = eduJSONArray.getJSONObject(i);
                                this.educations.add(new Education(eduJO.getString("id"), eduJO.getString("year"), eduJO.getString("schoolid"), eduJO.getString("departmentid"), eduJO.getString("level")));
                            }
                        }
                    }
                }
